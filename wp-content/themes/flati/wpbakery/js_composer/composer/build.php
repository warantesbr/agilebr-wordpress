...
class WPBakeryVisualComposerSetup extends WPBakeryVisualComposerAbstract {
    ...
    public function setUp() {
        ...
        if ( $vc_as_theme ) {
            ...
            $this->setUpPlugin();
            load_plugin_textdomain( 'ublflati', false,  get_template_directory() . '/languages/' );
        }
    ...
    public function addUpgradeMessageLink() {
        ...
        if(empty($username) || empty($api_key) || empty($purchase_code)) {
            echo ' <a href="'.$this->url.'">'.__('Dowload new version', 'ublflati').'</a> from CodeCanyon.';

        } else {
            echo ' <a href="'.wp_nonce_url( network_admin_url('options-general.php?page=wpb_vc_settings&vc_action=upgrade')).'">'.__('update now', 'ublflati').'</a> from CodeCanyon.';
        }
    }
...
class WPBakeryVisualComposerSetupAdmin extends WPBakeryVisualComposerSetup {
    ...
    public function adminNoticeNewVersion() {
        ...
        if(!empty($version))
            echo '<div class="updated"><p>'.sprintf(__('There is a new version of Visual Composer available. <a href="http://bit.ly/vcomposer" target="_blank">Download version <b>%s</b></a>.', 'ublflati'), $version).'</p></div>';
    }
    ...
    public function registerJavascript() {
        ...
        wp_localize_script( 'wpb_js_composer_js_view', 'i18nLocale', array(
            'add_remove_picture' => __( 'Add/remove picture', 'ublflati' ),
            'finish_adding_text' => __( 'Finish Adding Images', 'ublflati' ),
            'add_image' => __( 'Add Image', 'ublflati' ),
            'add_images' => __( 'Add Images', 'ublflati' ),
            'main_button_title' => __( 'Visual Composer', 'ublflati' ),
            'main_button_title_revert' => __( 'Classic editor', 'ublflati' ),
            'please_enter_templates_name' => __('Please enter templates name', 'ublflati'),
            'confirm_deleting_template' => __('Confirm deleting "{template_name}" template, press Cancel to leave. This action cannot be undone.', 'ublflati'),
            'press_ok_to_delete_section' => __('Press OK to delete section, Cancel to leave', 'ublflati'),
            'drag_drop_me_in_column' => __('Drag and drop me in the column', 'ublflati'),
            'press_ok_to_delete_tab' => __('Press OK to delete "{tab_name}" tab, Cancel to leave', 'ublflati'),
            'slide' => __('Slide', 'ublflati'),
            'tab' => __('Tab', 'ublflati'),
            'section' => __('Section', 'ublflati'),
            'please_enter_new_tab_title' => __('Please enter new tab title', 'ublflati'),
            'press_ok_delete_section' => __('Press OK to delete "{tab_name}" section, Cancel to leave', 'ublflati'),
            'section_default_title' => __('Section', 'ublflati'),
            'please_enter_section_title' => __('Please enter new section title', 'ublflati'),
            'error_please_try_again' => __('Error. Please try again.', 'ublflati'),
            'if_close_data_lost' => __('If you close this window all shortcode settings will be lost. Close this window?', 'ublflati'),
            'header_select_element_type' => __('Select element type', 'ublflati'),
            'header_media_gallery' => __('Media gallery', 'ublflati'),
            'header_element_settings' => __('Element settings', 'ublflati'),
            'add_tab' => __('Add tab', 'ublflati'),
            'are_you_sure_convert_to_new_version' => __('Are you sure you want to convert to new version?', 'ublflati'),
            'loading' => __('Loading...', 'ublflati'),
            // Media editor
            'set_image' => __('Set Image', 'ublflati'),
            'are_you_sure_reset_css_classes' => __('Are you sure taht you want to remove all your data?', 'ublflati'),
            'loop_frame_title' => __('Loop settings', 'ublflati'),
            'enter_custom_layout' => __('Enter custom layout for your row:', 'ublflati'),
            'wrong_cells_layout' => __('Wrong row layout format! Example: 1/2 + 1/2 or span6 + span6.', 'ublflati'),
        ) );
