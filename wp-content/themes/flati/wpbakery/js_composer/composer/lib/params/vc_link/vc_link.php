...
function vc_vc_link_form_field($settings, $value) {
    ...
    return '<div class="vc-link">'
        ...
        .'<a href="#" class="button vc-link-build '.$settings['param_name'].'_button">'.__('Select URL', 'ublflati').'</a> <span class="vc_link_label_title vc_link_label">'.__('Title', 'ublflati').':</span> <span class="title-label">'.$link['title'].'</span> <span class="vc_link_label">'.__('URL', 'ublflati').':</span> <span class="url-label">'.$link['url'].' '.$link['target'].'</span>'
        .'</div>';
}
