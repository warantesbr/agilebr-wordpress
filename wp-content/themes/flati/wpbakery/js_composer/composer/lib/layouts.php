...
class WPBakeryVisualComposerLayoutButton implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function setup($params) {
        if(empty($params['id']) || empty($params['title']))
            trigger_error( __("Wrong layout params", "ublflati"));
    ...
    public function output($post = null) {
        ...
        $output = '<li class="category-layout wpb-layout-element-button not-column-inherit_o"><a id="'.$this->params['id'].'" data-element="vc_column" data-width="'.$this->params['id'].'" class="'.$this->params['id'].' dropable_el clickable_action" href="#"><span>'.__($this->params['title'], "ublflati").'</span></a></li>';
        return $output;
    }
}
...
class WPBakeryVisualComposerTemplateMenuButton implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function output($post = null) {
        if(empty($this->params)) return '';
        $output = '<li class="wpb_template_li"><a data-template_id="'.$this->id.'" href="#">'.htmlspecialchars(__($this->params['name'], "ublflati")).'</a> <span class="wpb_remove_template" rel="'.$this->id.'"><i class="icon wpb_template_delete_icon"> </i> </span></li>';
        return $output;
    }
}
...
class WPBakeryVisualComposerElementButton implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function output($post = null) {
        ...
        $output .= '<li data-element="' . $this->base . '" class="wpb-layout-element-button'.$category_css_classes.$class_out.'"'.$data.'><a id="' . $this->base . '" class="dropable_el clickable_action'.$class.'" href="#">' . $this->getIcon() . htmlspecialchars(__(stripslashes($this->params["name"]), "ublflati")) .'</a></li>';
        return $output;
    }
}
...
class WPBakeryVisualComposerTemplateMenu implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function output( $only_list = false ) {
        ...
        if($only_list===false) {
            $output .=  '<li><ul>
                        <li id="wpb_save_template"><a href="#" id="wpb_save_template_button" class="button">'.__('Save current page as a Template', 'ublflati').'</a></li>
                        <li class="divider"></li>
                        <li class="nav-header">'.__('Load Template', 'ublflati').'</li>
                        </ul></li>
                        <li>
        ...
        if($is_empty) $output .= '<li class="wpb_no_templates"><span>'.__('No custom templates yet.', 'ublflati').'</span></li>';
        if($only_list===false) {
            $output .= '</ul></li>';
    ...
    public function getContentCategoriesLayouts() {
        ...
        foreach(WPBMap::getUserCategories() as $key => $name) {
            if($name === '_other_category_') {
                ...
                $output .='<li><a href="#" data-filter=".category-'.md5($name).'">'.__($name, "ublflati").'</a></li>';
            }
        }

        if($show_other) $output .= '<li><a href="#" data-filter=".category-'.$_other_category_index.'">'
                                    .__('Other', 'ublflati').'</a></li>';
        //$output .= '<li class="name_filter"><input id="vc_elements_name_filter" type="text" name="vc_content_filter" placeholder="'.__('Search by element name', 'ublflati').'"/></li>';
        $output .= '</ul></li>';
        return $output;
    }
    ...
    public function getElementsModal() {
        ...
        <div class="modal wpb-elements-list-modal">
          <div class="modal-header">
            ...
            <h3>'.__('Select Content Element', 'ublflati').'</h3>
            <input id="vc_elements_name_filter" type="text" name="vc_content_filter" placeholder="'.__('Search by element name', 'ublflati').'"/>
          </div>
          ...
          <div class="modal-footer hide">
            <button class="btn" data-dismiss="modal" aria-hidden="true">'.__('Close','ublflati').'</button>
          </div>
        </div>
        </script>
        ...
        <div class="modal wpb-element-edit-modal">
            ...
            <div class="modal-body wpb-edit-form">
                <div class="vc_row-fluid wpb-edit-form-inner">
                    <img src="images/wpspin_light.gif" alt="spinner" /> '.__('Loading...', 'ublflati').'
                </div>
            </div>
            <div class="modal-footer text-center">
                <button class="button-primary wpb_save_edit_form" data-dismiss="modal" aria-hidden="true">'.__('Save','ublflati').'</button>
                <button class="button" data-dismiss="modal" aria-hidden="true">'.__('Cancel','ublflati').'</button>
          </div>
        </div>
        </script>
        </div>';
            ...
            <div id="wpb_visual_composer-elements" class="vc_navbar">
                ...
                <div class="vc_navbar-inner clearfix">
                    <!--<div class="container">-->
                        <ul class="vc_nav">
                          <li>
                            <a title="'.__('Visual Composer for WordPress', 'ublflati').'" href="http://vc.wpbakery.com/" class="vc_logo" target="_blank"></a>
                          </li>
                        </ul>
                        '.$buttons.'
                            <ul class="vc_nav pull-right wpb-update-button">
                                <li><a class="button" id="wpb-save-post">'.__('Update', 'ublflati').'</a></li>
                            </ul>
                    <!--</div>-->
                </div>
            </div>';
    ...
    function buildButtons() {
        ...
        $buttons = array(
            array(
                ...
                '<ul class="vc_nav">
                                <li>
                                    <a class="wpb_add_new_element dropable_el button" id="wpb-add-new-element"><i class="icon"></i>'.__('Add element', 'ublflati').'</a>
                                </li>
                            </ul>'
            ),
            array(
                ...
                '<ul class="vc_nav">
                              <li>
                                <a class="wpb_add_new_row dropable_row button" id="wpb-add-new-row" data-element="vc_row"><i class="icon"></i>'.__('Add row', 'ublflati').'</a>
                                </li>
                            </ul>'
            ),
            array(
                ...
                '<ul class="vc_nav">
                                <li class="vc_dropdown">
                                    <a class="wpb_templates button"><i class="icon"></i>'.__('Templates', 'ublflati').' <b class="caret"></b></a>
...
class WPBakeryVisualComposerLayout implements  WPBakeryVisualComposerTemplateInterface {
	...
	public function getContainerHelper() {
		// return '<div class="container-helper">' . __('<h2>No content yet! You should add some...</h2>', 'ublflati') . __("<p>Use the buttons under <a href='javascript:open_elements_dropdown();' class='open-dropdown-content-element'><i class='icon'></i> Content Elements</a> on the top or add <a href='#' class='add-text-block-to-content'><i class='icon'></i> Text block</a> with single click.", 'ublflati') . '</p></div>';
        //return '<div class="container-helper"><span>' . __('<h2>No content yet! You should add some...</h2>', 'ublflati') . __('<p>Click <a href="#" class="add-element-to-layout" title="Add to this column"><i class="icon"></i></a> to add new element inside this column.', 'ublflati') . '</p></span></div>';
        return '';
    }
    ...
    public function output($post = null) {
        ...
        <div class="metabox-composer-content">
					<div id="wpb-convert-message">
					   <div class="messagebox_text"><p>'.__('Your page layout was created with previous Visual Composer version. Before converting your layout to the new version, make sure to <a target="_blank" href="http://kb.wpbakery.com/index.php?title=Update_Visual_Composer_from_3.4_to_3.5">read this page</a>.', 'ublflati').'</p>
					     <div class="wpb-convert-buttons">
					       <a class="wpb_convert button" id="wpb-convert"><i class="icon"></i>'.__('Convert to new version', 'ublflati').'</a>
					     </div>
					 </div>
				</div>
				
				<div class="vc_loading_block" style="display: none;">
				  <img src="'.get_site_url().'/wp-admin/images/wpspin_light.gif" /> '.__("Loading, please wait...", "ublflati").'
				</div>
				
				<div id="visual_composer_content" class="wpb_main_sortable main_wrapper"></div>
					<div id="wpb-empty-blocks">
					 <h2>' . __("No content yet! You should add some...", "ublflati") .'</h2>
					 <table class="helper-block">
					   <tr>
					     <td><span>1</span></td>
					     <td><p> '. __("This is a visual preview of your page. Currently, you don't have any content elements. Click or drag the button <a href='#' class='add-element-to-layout'><i class='icon'></i> Add element</a> on the top to add content elements on your page. Alternatively add <a href='#' class='add-text-block-to-content' parent-container='#visual_composer_content'><i class='icon'></i> Text block</a> with single click.", "ublflati") . '</p></td>
					   </tr>
					 </table>
					 <table class="helper-block">
					   <tr>
					     <td><span>2</span></td><td><p class="one-line"> '. __("Click the pencil icon on the content elements to change their properties.", "ublflati") . '</p></td>
					   </tr>
					   <tr>
					     <td colspan="2">
        ...
        $output .= '<input type="hidden" id="wpb_vc_loading" name="wpb_vc_loading" value="'. __("Loading, please wait...", "ublflati") .'" />';
        $output .= '<input type="hidden" id="wpb_vc_loading_row" name="wpb_vc_loading_row" value="'. __("Crunching...", "ublflati") .'" />';
