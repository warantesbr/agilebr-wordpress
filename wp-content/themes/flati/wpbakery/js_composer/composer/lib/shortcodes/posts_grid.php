...
class WPBakeryShortCode_VC_Posts_Grid extends WPBakeryShortCode {
    ...
    protected function getPostLink($grid_link, stdClass $post) {
        if($grid_link == 'link_post') {
            ...
            $title = sprintf( esc_attr__( 'Permalink to %s', 'ublflati' ), the_title_attribute( 'echo=0' ) );
            ...
            return array('url_image' => $url,
                ...
                'title_title' => sprintf( esc_attr__( 'Permalink to %s', 'ublflati' ), the_title_attribute( 'echo=0' ) ));
        }
        return false;
    }
